...
- 🏠 **브랜드 선택 페이지**: 4개 브랜드(MLB 25FW, MLB ACC, MLB KIDS, DISCOVERY) 요약 정보 카드
  - 각 브랜드별 전체원가율(USD), 총생산수량 YOY, 평균TAG YOY 표시
  - 평균원가(USD) 달러 기호 표시 (`$20.02`)
  - 환율 정보 표시 (`1,288 → 1,420 (110%)`)
  - 원가 상세보기(원부자재, 아트웍, 공임, 정상마진, 경비) 접기/펼치기 기능
- 📊 전체/카테고리별 원가율 분석
- 🔥 아이템별 원가 구성 히트맵
- 📈 시즌 간 비교 차트
- 🤖 AI 기반 인사이트 생성
...
### v1.5.0 (2025-01-XX) 🆕
- **첫 페이지 UI 개선**:
  - 평균원가(USD) 표시 형식: `20.02` → `$20.02` (달러 기호 추가)
  - 환율 표시 형식: `1,288 → 1,420 (110%)` (YOY 비율 표시, 100을 빼지 않고 표시)
  - 환율 YOY 스타일: 총 생산수량 YOY와 동일한 스타일로 통일 (`text-sm text-gray-600`)
  - "4개 브랜드" 버튼 제거
  - 날짜 텍스트 크기 축소: `25FW기간 (25.09~26.02)` 텍스트 크기 축소 (`text-sm`)
- **인사이트 텍스트 정리**:
  - 환노출/환헤지 관련 텍스트 모두 제거
  - USD 결제 벤더 구간 관리 항목 제거
  - 적용 위치: 워터폴 차트, 주요 지표 테이블, CSV 인사이트 파일, Python 스크립트
...
- **레이아웃 통일** 🆕:
  ...
  - 목적: 시각적 일관성 및 가독성 향상
- **레이아웃 확장 및 여백 최적화** 🆕:
  - 최대 너비 확장: `max-w-7xl` → `max-w-[98%]` (화면 공간 최대 활용)
  - 패딩 최적화: 헤더/메인/푸터 패딩 축소 (`py-8` → `py-4`, `px-8` → `px-4`)
  - 섹션 간격 축소: 컴포넌트 간 여백 최적화 (`mb-8` → `mb-4`, `mb-6` → `mb-4`)
  - 적용 위치:
    - 메인 페이지 (`app/page.tsx`): 헤더, 탭 메뉴, 메인 컨텐츠, 푸터
    - 원가율 카드 (`components/EnhancedStoryCards.tsx`): 섹션별 패딩 및 여백
    - 워터폴 차트 (`components/WaterfallChart.tsx`): 컨테이너 패딩 및 섹션 간격
    - 히트맵 테이블 (`components/Dashboard.tsx`): 컨테이너 패딩 및 여백
  - 목적: 빈 화면 공간 최소화 및 정보 밀도 향상으로 더 많은 콘텐츠를 한 화면에 표시
- **워터폴 차트 개선**:
