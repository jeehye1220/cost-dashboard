...
# -*- coding: utf-8 -*-
"""
SQL 데이터 연결 및 환율 변환 스크립트 (원산지명, 마감형태 포함)

SQL 쿼리를 실행하여 데이터를 추출하고, 발주통화에 따라 환율 변환을 적용하여
하나의 통합 CSV 파일(all_data.csv)을 생성합니다.
...
# ============================================
# SQL 쿼리 템플릿 (ORIGIN_NM, po_cls_nm 포함)
# ============================================
SQL_QUERY_TEMPLATE = """
with main as (
    select 
        ...
        o3.origin_nm as origin_nm,
        o3.po_cls_nm as po_cls_nm
    ...
    left join (
        select po_no, 
               max(origin_nm) as origin_nm,
               max(po_cls_nm) as po_cls_nm
        from prcs.dw_ord
        group by po_no
    ) o3 on o3.po_no = a.po_no
)
select 
    brd_cd as "브랜드",
    sesn as "시즌",
...
from main
where brd_cd in ({BRAND_PLACEHOLDER})  -- 브랜드 필터
    and sesn in ({SEASON_PLACEHOLDER})  -- 시즌 필터
group by 
    brd_cd, sesn, part_cd, vtext2, item_nm, po_no, tag_price, 
    po_qty_sum, cost_quotation_no, currency, mfac_compy_nm, origin_nm, po_cls_nm, quotation_submit_dt
...
# ============================================
OUTPUT_DIR = 'public'
OUTPUT_FILE = 'public/all_data.csv'
...
# SQL 쿼리 생성
# ============================================
def build_sql_query(brands: List[str], seasons: List[str]) -> str:
    """
    브랜드와 시즌 리스트를 받아서 SQL 쿼리 생성
    
    Args:
        brands: 브랜드 코드 리스트 (예: ['M'])
        seasons: 시즌 코드 리스트 (예: ['24S', '24F', '25S', '25F', '26S'])
    
    Returns:
        완성된 SQL 쿼리 문자열
    """
    # 브랜드 리스트를 SQL IN 절 형식으로 변환
    brand_list = "', '".join(brands)
    brand_placeholder = f"'{brand_list}'"
    
    # 시즌 리스트를 SQL IN 절 형식으로 변환
    season_list = "', '".join(seasons)
    season_placeholder = f"'{season_list}'"
    
    # SQL 쿼리 템플릿에 값 대입
    sql_query = SQL_QUERY_TEMPLATE.replace('{BRAND_PLACEHOLDER}', brand_placeholder)
    sql_query = sql_query.replace('{SEASON_PLACEHOLDER}', season_placeholder)
...
def save_csv(df: pd.DataFrame):
    ...
    print(f"\n[OK] {OUTPUT_FILE} 저장 완료 ({len(df)}개 행)")
    print("=" * 60)
...
def main():
    """메인 실행 함수"""
    # 명령줄 인자 파싱
    parser = argparse.ArgumentParser(description='SQL 데이터 연결 및 환율 변환 스크립트 (원산지명, 마감형태 포함)')
    parser.add_argument('--brand', nargs='+', default=['M', 'I', 'X', 'V', 'ST'],
                        help='브랜드 코드 리스트 (예: M 또는 M I X)')
    parser.add_argument('--season', nargs='+', 
                        default=['26SS', '26S', '25SS', '25S', '24SS', '24S', '25FW', '25F', '24FW', '24F'],
                        help='시즌 코드 리스트 (예: 24S 24F 25S 25F 26S)')
    ...
    print("SQL 데이터 연결 및 환율 변환 스크립트 (원산지명, 마감형태 포함)")
    print("=" * 60)
    ...
    # 1. SQL 쿼리 생성
    sql_query = build_sql_query(brands, seasons)
